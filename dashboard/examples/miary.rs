...
fn main() {
    ...
    for size in [19.0f32, 22.0, 27.0, 34.0, 44.0, 76.0] {
        for w in [Weight::Regular, Weight::Medium, Weight::Bold] {
            println!(
                "size {:>4} {:<8} lh={:>6.2} asc={:>6.2}",
                size,
                format!("{:?}", w),
                f.line_height(size, w),
                f.ascent(size, w)
            );
        }
    }
    ...
    for size in [19.0f32, 22.0, 27.0] {
        for w in [Weight::Medium, Weight::Bold] {
            ...
            let mut maxs = "";
            for s in [
                "pon", "wto", "śro", "czw", "pią", "sob", "nie", "P", "W", "Ś", "C", "P", "S", "N",
        ...
        for w in [Weight::Medium, Weight::Bold] {
            println!(
                "  3-lit: {:?} pon={:.2} śro={:.2} czw={:.2} pią={:.2}",
                w,
                f.measure("pon", size, w),
                f.measure("śro", size, w),
                f.measure("czw", size, w),
                f.measure("pią", size, w)
            );
            println!(
                "  2-lit: {:?} PN={:.2} WT={:.2} ŚR={:.2} CZ={:.2} PT={:.2} SB={:.2} ND={:.2}",
                w,
                f.measure("PN", size, w),
                f.measure("WT", size, w),
                f.measure("ŚR", size, w),
        ...
        for w in [Weight::Medium, Weight::Regular] {
            ...
            for t in tytuly {
                println!(
                    ...
                    f.measure(t, size, w),
                    t.chars().count(),
                    f.measure(t, size, w) / t.chars().count() as f32,
                    t
                );
            }
        }
    ...
    for size in [19.0f32, 22.0, 27.0, 34.0] {
        for w in [Weight::Regular, Weight::Medium, Weight::Bold] {
            ...
            println!(
                "size {:>4} {:<8} avg={:.3} px/zn",
                size,
                format!("{:?}", w),
                f.measure(proba, size, w) / n
            );
        }
    }
    ...
    for szer in [
        56i32, 60, 64, 66, 68, 72, 76, 84, 92, 100, 112, 120, 128, 136,
    ] {
        for size in [19.0f32, 22.0] {
    ...
    for size in [19.0f32, 22.0, 27.0, 34.0, 44.0] {
        for w in [Weight::Bold] {
            ...
            println!(
                "size {:>4} {:?} pelna_max={:.1} ({})  skrot_max={:.1} ({})",
                size, w, max, maxs, max3, max3s
            );
        }
    }
